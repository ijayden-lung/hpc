...
def mageckcount_getzerocounts(ctable):
  ...
  zerofactor=[0]*n
  for ni in range(n):
    nzr=[ (lambda x: 1 if x==0 else 0)(v[ni]) for (k,v) in list(ctable.items()) ]
...
def mageckcount_getmediannormfactor(ctable):
  ...
  n=len(ctable[list(ctable.keys())[0]]) # samples
  m=len(ctable) # sgRNAs
  meanval={k:math.exp( (sum( [ math.log(v2+1.0) for v2 in v])*1.0/n) ) for (k,v) in list(ctable.items()) if sum(v)>0}  # geometric mean
  meanval={k:(lambda x: x if x>0 else 1)(v) for (k,v) in list(meanval.items())} # delete those with all 0 read counts
  #samplefactor=[0]*n
  medianfactor=[0.0]*n
  for ni in range(n):
    meanfactor=[ v[ni]/meanval[k] for (k,v) in list(ctable.items()) if k in meanval]
    #print(str(sorted(meanfactor)))
...
def normalizeCounts(ctable,sgdict=None,method='median',returnfactor=False,reversefactor=False,controlsgfile=None,norm_msgs=None):
  ...
  if method=='control':
    ...
    ctable_nm={k:v for (k,v) in list(ctable.items()) if k in controlsglist}
    if len(ctable_nm)==0 and sgdict !=None: # try to search in sgdict
      ctable_nm={k:v for (k,v) in list(ctable.items()) if sgdict[k][0] in controlsglist}
  ...
  if returnfactor:
    if reversefactor:
      ...
      return samplefactor
  # normalize the table
  ntable={ k: [ samplefactor[i]*v[i] for i in range(n)] for (k,v) in list(ctable.items())}
  return ntable
