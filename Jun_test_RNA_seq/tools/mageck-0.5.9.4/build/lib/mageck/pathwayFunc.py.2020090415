...
def mageck_pathwayrra_onedir(args,pdict,cid,sourcefile,rra_path_input_file,rra_path_output_file):
  ...
  ginfo_st=sorted(list(ginfo.items()),key=lambda x: x[1])
  nthreshold=0
...
def mageck_pathway_standardize(gdict):
  ...
  ginfo_rt={ k:((v-ginfo_med)/ginfo_std) for (k,v) in list(gdict.items())}
  return ginfo_rt
...
def mageck_pathway_ztest_permutation(args,gdict,pdict,pdictpvals):
  ...
  for iter in range(niter):
    ...
    for (pname,pitem) in list(pdictpvals.items()):
      nsize=pitem[2]
  ...
  pdictqvals={k:(v[0]*1.0/niter,v[1]*1.0/niter) for (k,v) in list(pdictqvals.items())}
  return pdictqvals
...
def mageck_pathway_ztest(args,gdict,pdict):
  ...
  '''
  pdictpvals={}
  for (pname,pitem) in list(pdict.items()):
...
def mageck_pathwaygsa(args):
  ...
  if args.gene_ranking_2 is not None:
    ...
    for gk in list(ginfo.keys()):
      if gk in list(ginfo2.keys()):
  ...
  pathway_qval_tup=sorted(list(pathway_qval.items()),key=lambda x:min(x[1]))
