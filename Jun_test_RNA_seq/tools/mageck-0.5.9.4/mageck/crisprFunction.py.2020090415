...
def magecktest_main(args):
  ...
  for cpindex in range(len(supergroup_treat)):
    ...
    if cpindex==0 and len(supergroup_treat)==1:
    ...
    if cpindex==0 and len(supergroup_treat)>1: # for more than 1 comparisons, set up the basic gene_summary.txt
      # for more than 1 -t and -c pairs, use the results of the first pair as the default sgrna_summary.txt and gene_summary.txt 
